...
class UserProfileHeader: UICollectionViewCell {
    ...
    fileprivate func  setupEditFollowButton(){
        ...
        if currentUser == userId {
            ...
            // check if following
            FIRDatabase.database().reference().child("following").child(currentUser).child(userId).observeSingleEvent(of: .value, with: { (snapshot) in
                
                if let isFollowing = snapshot.value as? Int, isFollowing == 1 {
                    
                    self.editProfileFollowButton.setTitle("언팔로우", for: .normal)
    ...
    func handleEditProfileOrFollow(){
        ...
        if editProfileFollowButton.titleLabel?.text == "언팔로우"{
            //unfollow
    ...
    lazy var gridButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(handleChangedToGridView), for: .touchUpInside)
        return button
    }()
    ...
    func handleChangedToGridView(){
        print("handleChangedToGridView")
    ...
    func handleChangedToListView(){
        print("handleChangedToListView")
